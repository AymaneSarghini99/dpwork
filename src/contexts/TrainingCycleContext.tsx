...
import {
  ...
  type TrainingCycleInfo,
} from '@/lib/trainingCycle';

type TrainingCycleContextValue = {
  ready: boolean;
  cycleInfo: TrainingCycleInfo;
  activeProgram: TrainingProgram;
  programOverride: ProgramOverride;
  cycleStartDate: string;
  saveProgramMode: (override: ProgramOverride) => Promise<void>;
};

const TrainingCycleContext = createContext<TrainingCycleContextValue | null>(
  null
);

export const TrainingCycleProvider = ({ children }: { children: ReactNode }) => {
  const { user } = useAuth();
  ...
  const activeProgram = cycleInfo.activeProgram as TrainingProgram;

  const saveProgramMode = useCallback(
    async (override: ProgramOverride) => {
      if (!user) {
  ...
  return (
    <TrainingCycleContext.Provider value={value}>
      {children}
    </TrainingCycleContext.Provider>
  );
};

export const useTrainingCycle = (): TrainingCycleContextValue => {
  const ctx = useContext(TrainingCycleContext);
  if (!ctx) {
    throw new Error('useTrainingCycle must be used within TrainingCycleProvider');
  }
  return ctx;
};
